...
export class DataTableComponent implements OnInit {
	// Public properties
	dataSource: any;
	displayedColumns = ['id', 'date', 'desc', 'amt', 'avbal'];
	...
	ngOnInit() {
		...
		**/
		...
		// Init DataSource
		// this.dataSource = [
		// 	{ id: 1, date: 'Hydrogen', desc: 1.0079, amt: 'H', avbal: 0 }
		// ];
		// First load
		// this.loadItems(true);
	}

	public parseTransactions(transactions: any) {
		let dataSource: Array<any> = new Array<any>();
		transactions.forEach(transaction => {
			let date = new Date(transaction.valueDateTime);
			let record = 
			{ 
				id: transaction.transactionId,
				date: date.getDate() + '/' + (date.getMonth()+1) + '/' + date.getFullYear(),
				desc: transaction.purpose,
				amt: (transaction.balanceCreditDebitIndicator!='Credit' ? transaction.transactionAmount * -1 : transaction.transactionAmount) ,
				avbal: transaction.balance
			};
